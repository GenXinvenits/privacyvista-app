                     ...
                     JSON_ARRAY_APPEND(
                       JSON_ARRAY_APPEND(
                         JSON_ARRAY_APPEND(
                           ...
                           '$.sections[4].fields', JSON_OBJECT('key','dpia_assistance','label','Processor Assists Controller with DPIA','type','select','options',JSON_ARRAY('yes','no','not_applicable'))),
                         '$.sections[4].fields', JSON_OBJECT('key','dsar_support','label','DSAR Support Process Established','type','select','options',JSON_ARRAY('yes','no','not_applicable'))),
                       '$.sections[4].fields', JSON_OBJECT('key','data_subject_requests','label','Data Subject Requests / Complaints Record','type','textarea')),
                     '$.sections[4].fields', JSON_OBJECT('key','breach_management','label','Personal Data Breach / Incident Management','type','textarea')),
                   '$.sections[4].fields', JSON_OBJECT('key','last_audit_date','label','Last Audit / Review Date','type','date')),
                 '$.sections[4].fields', JSON_OBJECT('key','last_audit_performed_by','label','Last Audit Performed By','type','text')),
               '$.sections[4].fields', JSON_OBJECT('key','next_review_date','label','Next Scheduled Review Date','type','derived_date','derived_from','last_audit_date','offset_months',11)),
             '$.sections[4].fields', JSON_OBJECT('key','approver','label','Approver (Management / DPO)','type','text')),
           '$.sections[4].fields', JSON_OBJECT('key','remarks','label','Remarks / Notes','type','textarea')),
