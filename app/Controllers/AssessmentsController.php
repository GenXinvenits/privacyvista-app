...
class AssessmentsController extends BaseController
{
 ...
 public function index():void{$this->requireLogin();$c=$this->client();if(!$c){http_response_code(404);exit('Client not found');}$assessments=(new Assessment())->forClient((int)$c['id']);$this->view('assessments/index',['client'=>$c,'assessments'=>$assessments]);}
 public function create():void{$this->requireLogin();$c=$this->client();if(!$c){http_response_code(404);exit('Client not found');}$this->view('assessments/form',['client'=>$c,'assessment'=>null,'activities'=>$this->formData((int)$c['id']),'title'=>'New privacy assessment']);}
