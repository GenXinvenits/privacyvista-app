...
class TasksController extends BaseController{
 ...
 public function create():void{$this->requireLogin();$c=$this->client();if(!$c){http_response_code(404);exit('Client not found');}$db=Database::connect();$ass=$db->prepare('SELECT id,title FROM privacy_assessments WHERE client_id=? ORDER BY created_at DESC');$ass->execute([$c['id']]);$users=$db->query('SELECT id,name FROM users ORDER BY name')->fetchAll();$this->view('tasks/form',['client'=>$c,'assessments'=>$ass->fetchAll(),'users'=>$users]);}
 ...
 public function status():void{$this->requireLogin();if(!Security::verifyCsrf($_POST['_csrf']??null)){http_response_code(419);exit('Invalid security token');}$c=$this->client();if($c){$status=$_POST['status']??'open';if(!in_array($status,['open','in_progress','blocked','completed','cancelled'],true))$status='open';(new PrivacyTask())->updateStatus((int)$_POST['id'],(int)$c['id'],$status);}redirect('tasks?client_id='.(int)($c['id']??0));}
}
