...
use App\Models\AdminClientAccess;
...
class FormsController extends BaseController
{
 ...
 public function templateEdit():void{$this->requireLogin();if(!$this->isSuperuser()){http_response_code(403);exit('Superuser access required');}$slug=trim((string)($_GET['slug']??''));$template=(new FormTemplate())->findBySlug($slug);if(!$template){http_response_code(404);exit('Form template not found');}$active=(new FormTemplate())->active($slug);$this->view('forms/template-editor',['title'=>'Edit '.$template['name'].' template','template'=>$template,'version'=>$active]);}
 ...
 public function ropa():void{$this->requireLogin();$client=$this->resolveClient();$template=(new FormTemplate())->active('ropa');if(!$template){http_response_code(500);exit('ROPA form template is not configured');}$clients=$this->assignedClients();$records=(new Ropa())->forClient((int)$client['id']);$id=(int)($_GET['id']??0);$ropa=[];$record=null;if($id>0){$record=(new Ropa())->find($id);if(!$record){http_response_code(404);exit('ROPA record not found');}if((int)$record['client_id']!==(int)$client['id']||!$this->canAccessFormClient((int)$record['client_id'])){http_response_code(403);$this->view('errors/403',['title'=>'Access denied','message'=>'You do not have access to this ROPA record.']);exit;}$ropa=$record['data'];$ropa['id']=$record['id'];$ropa['client_id']=$record['client_id'];$ropa['process_name']=$record['process_name'];$ropa['status']=$record['status'];$templateForRecord=$record['form_definition'];if($templateForRecord)$template['definition']=$templateForRecord;$template['version_number']=(int)($record['form_version_number']??$template['version_number']);$_SESSION['ropa_edit_id']=$record['id'];}else{unset($_SESSION['ropa_edit_id']);}$this->view('forms/ropa',compact('client','clients','records','record','ropa','template'));}
 public function ropaStore():void{$this->requireLogin();if(!Security::verifyCsrf($_POST['_csrf']??null)){http_response_code(419);exit('Invalid security token');}$client=$this->resolveClient();$processName=trim((string)($_POST['process_name']??''));if($processName===''){http_response_code(422);exit('Processing activity / process is required');}$payload=$_POST;unset($payload['_csrf']);$payload['client_id']=(int)$client['id'];$model=new Ropa();$id=(int)($_POST['id']??($_SESSION['ropa_edit_id']??0));$data=['client_id'=>(int)$client['id'],'process_name'=>$processName,'status'=>trim((string)($_POST['status']??'planned')),'payload'=>$payload,'created_by'=>(int)($_SESSION['user']['id']??0)];if($id>0){$record=$model->find($id);if(!$record||!$this->canAccessFormClient((int)$record['client_id'])||(int)$record['client_id']!==(int)$client['id']){http_response_code(403);exit('Access denied');}$data['id']=$id;$model->update($data);}else{$template=(new FormTemplate())->active('ropa');if(!$template){http_response_code(500);exit('ROPA form template is not configured');}$data['form_version_id']=(int)$template['active_version_id'];$id=$model->create($data);}$_SESSION['ropa_edit_id']=$id;redirect('forms/ropa?client_id='.$client['id'].'&id='.$id);}
}
