...
<?php elseif(in_array($type,['auto','auto_date'],true)): ?><input class="form-control" id="<?= e($key) ?>" value="<?= $type==='auto_date' ? ($value($key) ?: e(date('Y-m-d',strtotime('+11 months')))) : ($key==='record_id' ? e(!empty($ropa['id'])?'ROPA-'.(int)$ropa['id']:'Generated when saved') : $value($key)) ?>" readonly>
