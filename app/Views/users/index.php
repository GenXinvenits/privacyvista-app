    ...
    <div class="table-responsive"><table class="table table-hover align-middle">
        ...
        <?php else: foreach ($users as $user): $isSuperuser = strtolower((string)($user['role'] ?? '')) === 'superuser'; $canManage = $this->isSuperuser() || !$isSuperuser; ?><tr>
            ...
            <td class="text-nowrap">
                <?php if ($canManage): ?>
                    <a href="<?= url('users/edit', ['id' => $user['id']]) ?>" class="btn btn-sm btn-outline-secondary">Edit</a>
                    <form method="post" action="<?= url('users/delete') ?>" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this user?');"><?php include __DIR__ . '/../partials/csrf.php'; ?><input type="hidden" name="id" value="<?= (int)$user['id'] ?>"><button type="submit" class="btn btn-sm btn-outline-danger">Delete</button></form>
                <?php else: ?>
                    <button type="button" class="btn btn-sm btn-outline-secondary" disabled title="Admins cannot edit Superusers">Edit</button>
                    <button type="button" class="btn btn-sm btn-outline-danger" disabled title="Admins cannot delete Superusers">Delete</button>
                <?php endif; ?>
            </td>
