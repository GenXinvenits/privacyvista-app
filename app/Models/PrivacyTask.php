...
{
 public function forClient(int $id):array{$s=$this->db->prepare('SELECT t.*,u.name AS assignee_name FROM privacy_tasks t LEFT JOIN users u ON u.id=t.assigned_to WHERE t.client_id=? ORDER BY t.due_date IS NULL,t.due_date,t.id DESC');$s->execute([$id]);return$s->fetchAll();}
